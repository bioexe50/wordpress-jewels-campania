...
class acf_field_thsn_fonticonpicker extends acf_field {
	...
	function __construct() {
		...
		$this->label = esc_html__('Icon Picker', 'itinc');
		$this->category = esc_attr__("jQuery", 'itinc');

		parent::__construct();

    	// Settings
		$this->settings = array(
			'dir' 		=>  get_template_directory_uri() . '/includes/acf/themesion-acf-iconpicker/',
			'path'		=>	get_template_directory_uri() . '/includes/acf/themesion-acf-iconpicker/',
			'config' 	=>  get_template_directory_uri() . '/includes/acf/themesion-acf-iconpicker/icons/config.json', //	plugins_url('icons/config.json', __FILE__),
			'icons'		=>	get_template_directory_uri() . '/includes/acf/themesion-acf-iconpicker/icons/css/fontello.css', // plugins_url('icons/css/fontello.css', __FILE__),
			'version' 	=> 	'1.0.0',
			'library' 	=> 	'fontawesome'
		);
	...
	function render_field( $field ) {
		...
		echo '<option value="">'.esc_html__('None', 'itinc').'</option>';
		echo '</select>';
	...
	function input_admin_enqueue_scripts() {
		...
		wp_enqueue_script( 'acf-thsn_fonticonpicker-iconarray', get_template_directory_uri() . '/includes/customizer/themesion-icon-picker/themesion-icon-picker.js', array('jquery','acf-thsn_fonticonpicker'), $this->settings['version'] );
