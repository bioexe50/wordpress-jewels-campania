...
<div class="wrap">
    <h1 class="wp-heading-inline">
        📅 <?php _e('Gestione Prenotazioni', 'aule-booking'); ?>
    </h1>
    ...
    <?php if (!empty($_GET['updated'])): ?>
        <div class="notice notice-success is-dismissible">
            <p>
                ...
                switch ($_GET['updated']) {
                    case 'approved':
                        _e('Prenotazione approvata correttamente.', 'aule-booking');
                        break;
                    case 'rejected':
                        _e('Prenotazione rifiutata correttamente.', 'aule-booking');
                        break;
                    case 'deleted':
                        _e('Prenotazione eliminata correttamente.', 'aule-booking');
                        break;
                    default:
    ...
    <div class="prenotazioni-stats-grid">
        <div class="stat-card total">
            ...
            <div class="stat-label"><?php _e('Totali', 'aule-booking'); ?></div>
        </div>
        <div class="stat-card pending">
            <div class="stat-number"><?php echo $stats['in_attesa']; ?></div>
            <div class="stat-label"><?php _e('In Attesa', 'aule-booking'); ?></div>
        </div>
        <div class="stat-card confirmed">
            <div class="stat-number"><?php echo $stats['confermate']; ?></div>
            <div class="stat-label"><?php _e('Confermate', 'aule-booking'); ?></div>
        </div>
        <div class="stat-card rejected">
            <div class="stat-number"><?php echo $stats['rifiutate']; ?></div>
            <div class="stat-label"><?php _e('Rifiutate', 'aule-booking'); ?></div>
        </div>
        <div class="stat-card today">
            <div class="stat-number"><?php echo $stats['oggi']; ?></div>
            <div class="stat-label"><?php _e('Oggi', 'aule-booking'); ?></div>
        </div>
    </div>
    ...
    <div class="prenotazioni-container">
        ...
        <div class="prenotazioni-filters">
            <form method="get" class="prenotazioni-filter-form">
                <input type="hidden" name="page" value="aule-booking-prenotazioni">

                <select name="stato" class="filter-prenotazioni">
                    <option value=""><?php _e('Tutti gli stati', 'aule-booking'); ?></option>
                    <option value="in_attesa" <?php selected($filter_stato, 'in_attesa'); ?>><?php _e('In Attesa', 'aule-booking'); ?></option>
                    <option value="confermata" <?php selected($filter_stato, 'confermata'); ?>><?php _e('Confermate', 'aule-booking'); ?></option>
                    <option value="rifiutata" <?php selected($filter_stato, 'rifiutata'); ?>><?php _e('Rifiutate', 'aule-booking'); ?></option>
                    <option value="cancellata" <?php selected($filter_stato, 'cancellata'); ?>><?php _e('Cancellate', 'aule-booking'); ?></option>
                </select>

                <select name="aula_id" class="filter-prenotazioni">
                    <option value=""><?php _e('Tutte le aule', 'aule-booking'); ?></option>
                ...
                <input type="date"
                       ...
                       placeholder="<?php _e('Data da', 'aule-booking'); ?>">

                <input type="date"
                       name="data_a"
                       value="<?php echo esc_attr($filter_data_a); ?>"
                       placeholder="<?php _e('Data a', 'aule-booking'); ?>">

                <input type="search"
                       name="search"
                       value="<?php echo esc_attr($search_term); ?>"
                       placeholder="<?php _e('Cerca per nome, email, codice...', 'aule-booking'); ?>">
        ...
        <?php if (empty($prenotazioni)): ?>
            ...
            <div class="no-data">
                📅
                <h3><?php _e('Nessuna prenotazione trovata', 'aule-booking'); ?></h3>
                <p><?php _e('Non ci sono prenotazioni che corrispondono ai criteri di ricerca.', 'aule-booking'); ?></p>
            </div>
        <?php else: ?>
            <!-- Contatore risultati -->
            <div class="prenotazioni-results-info">
                <span class="prenotazioni-count">
                    <?php printf(__('Mostrando %d prenotazioni', 'aule-booking'), count($prenotazioni)); ?>
                </span>
            </div>
            ...
            <div class="prenotazioni-table-container">
                <table class="wp-list-table widefat fixed striped prenotazioni-table">
                    <thead>
                        <tr>
                            <th class="column-codice"><?php _e('Codice', 'aule-booking'); ?></th>
                            <th class="column-richiedente"><?php _e('Richiedente', 'aule-booking'); ?></th>
                            <th class="column-aula"><?php _e('Aula', 'aule-booking'); ?></th>
                            <th class="column-data"><?php _e('Data e Ora', 'aule-booking'); ?></th>
                            <th class="column-stato"><?php _e('Stato', 'aule-booking'); ?></th>
                            <th class="column-azioni"><?php _e('Azioni', 'aule-booking'); ?></th>
                        </tr>
                    </thead>
                    <tbody>
                            ...
                            <tr class="prenotazione-row" data-id="<?php echo $prenotazione->id; ?>">
                                <td class="column-codice">
                                    ...
                                    <div class="row-actions">
                                        <span class="created">
                                            <?php printf(__('Creata: %s', 'aule-booking'),
                                ...
                                <td class="column-stato">
                                    <span class="booking-status booking-status-<?php echo esc_attr($prenotazione->stato); ?>">
                                        ...
                                        switch ($prenotazione->stato) {
                                            case 'in_attesa':
                                                echo '⏳ ' . __('In Attesa', 'aule-booking');
                                                break;
                                            case 'confermata':
                                                echo '✅ ' . __('Confermata', 'aule-booking');
                                                break;
                                            case 'rifiutata':
                                                echo '❌ ' . __('Rifiutata', 'aule-booking');
                                                break;
                                            case 'cancellata':
                                                echo '🚫 ' . __('Cancellata', 'aule-booking');
                                                break;
                                        }
                                        ?>
                                    </span>
                                    ...
                                    <div class="booking-actions">
                                        <?php if ($prenotazione->stato === 'in_attesa'): ?>
                                            <button class="button button-small approve-booking"
                                                    ...
                                                    title="<?php _e('Approva prenotazione', 'aule-booking'); ?>">
                                                ✅ <?php _e('Approva', 'aule-booking'); ?>
                                            </button>

                                            <button class="button button-small reject-booking"
                                                    data-id="<?php echo $prenotazione->id; ?>"
                                                    title="<?php _e('Rifiuta prenotazione', 'aule-booking'); ?>">
                                                ❌ <?php _e('Rifiuta', 'aule-booking'); ?>
                                            </button>
                                        <?php endif; ?>

                                        <button class="button button-small view-details"
                                                data-id="<?php echo $prenotazione->id; ?>"
                                                title="<?php _e('Visualizza dettagli', 'aule-booking'); ?>">
                                            👁️ <?php _e('Dettagli', 'aule-booking'); ?>
                                        </button>

                                        <?php if (in_array($prenotazione->stato, ['rifiutata', 'cancellata'])): ?>
                                        <button class="button button-small delete-booking"
                                                data-id="<?php echo $prenotazione->id; ?>"
                                                title="<?php _e('Elimina prenotazione', 'aule-booking'); ?>">
                                            🗑️ <?php _e('Elimina', 'aule-booking'); ?>
                                        </button>
                                        <?php endif; ?>
                                    </div>
                                </td>
...
<div id="booking-details-modal" style="display: none;">
    <div class="modal-content">
        <div class="modal-header">
            <h3><?php _e('Dettagli Prenotazione', 'aule-booking'); ?></h3>
...
<div id="reject-booking-modal" style="display: none;">
    <div class="modal-content">
        <div class="modal-header">
            <h3><?php _e('Rifiuta Prenotazione', 'aule-booking'); ?></h3>
            <button class="modal-close">&times;</button>
        </div>
        <div class="modal-body">
            <p><?php _e('Inserisci il motivo del rifiuto:', 'aule-booking'); ?></p>
            <textarea id="reject-reason" rows="4" style="width: 100%;"
                      placeholder="<?php _e('Es: Aula non disponibile per manutenzione', 'aule-booking'); ?>"></textarea>
            <div class="modal-actions">
                <button class="button button-primary confirm-reject"><?php _e('Conferma Rifiuto', 'aule-booking'); ?></button>
                <button class="button cancel-reject"><?php _e('Annulla', 'aule-booking'); ?></button>
            </div>
        </div>
    </div>
</div>
...
jQuery(document).ready(function($) {
    ...
    $('.approve-booking').on('click', function() {
        ...
        if (!confirm('<?php esc_js(_e('Confermare questa prenotazione?', 'aule-booking')); ?>')) {
            return;
        }
        ...
        $.ajax({
            ...
            data: {
                ...
                note_admin: '',
                nonce: '<?php echo wp_create_nonce('aule_booking_admin_nonce'); ?>'
            },
            ...
            error: function() {
                alert('<?php esc_js(_e('Errore di comunicazione', 'aule-booking')); ?>');
            }
        });
    });
    ...
    $('.confirm-reject').on('click', function() {
        ...
        if (!reason) {
            alert('<?php esc_js(_e('Il motivo del rifiuto è obbligatorio', 'aule-booking')); ?>');
            return;
        }
    ...
    $('.delete-booking').on('click', function() {
        ...
        if (!confirm('<?php esc_js(_e('Eliminare definitivamente questa prenotazione?', 'aule-booking')); ?>')) {
            return;
        }
