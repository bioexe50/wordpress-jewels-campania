...
class Prenotazione_Aule_SSM_Cron {
     ...
     * @since 1.0.0
     * @access private
     * @var Prenotazione_Aule_SSM_Email $email
     */
    private $email;
    ...
    public function __construct($plugin_name, $version) {
        ...
        $this->email = new Prenotazione_Aule_SSM_Email();
    }
    ...
    public function cleanup_expired_bookings() {
        ...
        do_action('prenotazione_aule_ssm_after_cleanup', $total_processed);
    }
    ...
    public function send_booking_reminders() {
        ...
        foreach ($bookings as $booking) {
            ...
            $reminder_sent = get_transient('prenotazione_aule_ssm_reminder_sent_' . $booking->id);
            ...
            if (!$reminder_sent) {
                try {
                    ...
                    if ($result) {
                        ...
                        set_transient('prenotazione_aule_ssm_reminder_sent_' . $booking->id, true, 48 * HOUR_IN_SECONDS);
                    } else {
                        $failed_count++;
                    }
        ...
        // Hook per estensioni personalizzate
        do_action('prenotazione_aule_ssm_after_reminders', $sent_count, $failed_count);
    }
    ...
    public function send_weekly_report() {
        ...
        // Hook per estensioni personalizzate
        do_action('prenotazione_aule_ssm_after_weekly_report', $result);
    }
    ...
    public function check_database_integrity() {
        ...
        // Hook per estensioni personalizzate
        do_action('prenotazione_aule_ssm_after_integrity_check', $issues);
    }
    ...
    public function update_stats_cache() {
        try {
            ...
            set_transient('prenotazione_aule_ssm_dashboard_stats', $stats, HOUR_IN_SECONDS);

            // Calcola statistiche mensili
            $monthly_stats = $this->calculate_monthly_stats();
            set_transient('prenotazione_aule_ssm_monthly_stats', $monthly_stats, HOUR_IN_SECONDS);
    ...
    private function handle_cron_error($context, $message) {
        ...
        $error_count = get_transient('prenotazione_aule_ssm_cron_errors') ?: 0;
        $error_count++;
        set_transient('prenotazione_aule_ssm_cron_errors', $error_count, DAY_IN_SECONDS);

        // Se ci sono troppi errori, invia notifica
        if ($error_count >= 10) {
            $this->send_error_notification($error_count);
            delete_transient('prenotazione_aule_ssm_cron_errors');
        }
    }
