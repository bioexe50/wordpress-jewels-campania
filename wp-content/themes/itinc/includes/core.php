...
function thsn_element_template_list( $for='portfolio', $section=true ){
	$return = array();
	if( !empty($for) ){
		// Default titles
		...
		$elements_array = array(
			'icon-heading'				=> array( 'name' => esc_attr__('Icon Heading', 'itinc'),			
				'total_styles' 			=> 18,
			...
			'testimonial'				=> array( 'name' => esc_attr__('Testimonial', 'itinc'),			
				'total_styles' 			=> 4,
				'exclude_in_customizer' => array(), // add style number here
			 ),
			'client'					=> array( 'name' => esc_attr__('Client', 'itinc'),				
				'total_styles' 			=> 2,
				'exclude_in_customizer' => array(), // add style number here
		 	),
			'blog'						=> array( 'name' => esc_attr__('Blog', 'itinc'),					
				'total_styles'			=> 2,
				'exclude_in_customizer' => array(), // add style number here
		 	),
			'pricing-table'				=> array( 'name' => esc_attr__('Pricing Table', 'itinc'),			
				'total_styles' 			=> 1,
				'exclude_in_customizer' => array(), // add style number here
		 	),
			'facts-in-digits'			=> array( 'name' => esc_attr__('Facts In Digits', 'itinc'),		
				'total_styles' 			=> 6,
				'exclude_in_customizer' => array(), // add style number here	
		 	),
			'static-box'				=> array( 'name' => esc_attr__('Static Box', 'itinc'),			
				'total_styles' 			=> 2,
				'exclude_in_customizer' => array(), // add style number here
		 	),
			'opening-hours-list'		=> array( 'name' => esc_attr__('Opening Hours List', 'itinc'),	
				'total_styles' 			=> 2,
				'exclude_in_customizer' => array(), // add style number here
		 	),
		);
...
function thsn_social_links_list( $settings = array( 'position' => 'start', 'column' => '3' ) ){
	return array(
		...
		array(
			'id'			=> 'twitter',
			'label'			=> 'Twitter',
			'icon_class'	=> 'thsn-base-icon-twitter-x',
		),
		array(
			'id'			=> 'linkedin',
			'label'			=> 'LinkedIn',
...
function thsn_footer_copyright_area() {
	...
	if( !empty($footer_copyright_text) ){
		$footer_copyright_content[] = '<div class="thsn-footer-copyright-text-area"> ' . thsn_esc_kses($footer_copyright_text) . '</div>';
	}
...
function thsn_footer_boxes_area() {
	...
	if( $footer_boxes_area == true ){
		...
		for( $x=1; $x<=3; $x++ ){
			...
			$link		= thsn_get_base_option('footer-box-'.$x.'-link');
			...
			$footer_box_link_start = '';
			$footer_box_link_end = '';

			if( !empty($title) ){
				if( !empty($link) ){
					$footer_box_link_start = '<a href="'.esc_url($link).'">';
					$footer_box_link_end = '</a>';
				}
				$footer_box_content[] = $footer_box_link_start . '<span class="thsn-footer-contact-info"><span class="thsn-footer-contact-info-inner">
				' . thsn_esc_kses($icon_html) . '
				<span class="thsn-label thsn-label-'.esc_attr($x).'">'.esc_html($title).'</span> '.esc_html($desc).'
			</span></span>' . $footer_box_link_end;
			}
		}
...
if ( !function_exists( 'thsn_header_slider' ) ){
	function thsn_header_slider(){
		...
		if( is_page() || is_singular() || is_home() ){
			...
			$slider_type = get_post_meta( $page_id, 'thsn-slider-type', true );
			if( !empty($slider_type) ){
				// Check if Slider Revolution
				if( $slider_type=='revolution-slider' ){
					$slider_slug = get_post_meta( $page_id, 'thsn-revolution-slider', true );
					if( !empty($slider_slug) && function_exists('add_revslider') ){
						...
						$below_content = get_post_meta( $page_id, 'thsn-slider-below-content', true );
						if( !empty($below_content) ){
							echo thsn_esc_kses('<div class="container thsn-slider-bottom-section"><div class="row">'.$below_content.'<div class="col-sm-5"></div></div></div>');
						}
					}
				} else if( $slider_type=='custom-code' ){
					$custom_slider_code = get_post_meta( $page_id, 'thsn-custom-slider-code', true );
	...
	if( $featured_img_only==true || !in_array($post_format, array('audio', 'video', 'gallery', 'quote', 'link')) ){
		...
		switch( $post_format ){
			...
			case 'gallery' :
				...
				$images = get_post_meta( $post_id, 'thsn-photo-gallery', true );
				$images_pformat = get_post_meta( $post_id, 'thsn-pformat-gallery', true );
...
function thsn_element_block_container( $settings = array( 'position' => 'start', 'column' => '3', 'cpt' => 'blog', 'taxonomy' => 'category', 'style' => '1', 'odd_even' => '', 'col_odd_even' => '' ) ){
	...
	if( is_array($terms) && count($terms)>0 ){
		foreach( $terms as $term ){
			$term_slug .= $term->slug.' thsn-term-' . $term->term_id . ' ' ;
		}
...
function thsn_titlebar_headings(){
	...
	if( is_singular() || is_home() ){
		if( is_home() ){
			...
			$page_id	= get_option( 'page_for_posts' );
			$current_single_title		= get_post_meta( $page_id, 'thsn-titlebar-title', true );
			$current_single_subtitle	= get_post_meta( $page_id, 'thsn-titlebar-subtitle', true );
...
if( !function_exists('thsn_header_contactinfo') ){
function thsn_header_contactinfo( $settings = array() ){
	$line1	= thsn_get_base_option('header-contactinfo-line1');
	$line2	= thsn_get_base_option('header-contactinfo-line2');
	$url	= thsn_get_base_option('header-bcontactinfo-link');
	
	$return = '';
	if( !empty($line1) ){
		$return .= thsn_esc_kses('<span class="thsn-header-button-text-1">'.esc_html($line1).'</span>');
	}
	if( !empty($line2) ){
		$return .= thsn_esc_kses('<span class="thsn-header-button-text-2">'.esc_html($line2).'</span>');
	}

	if( !empty($url) && !empty($return) ){
		$return = '<a href="'.esc_url($url).'">'.thsn_esc_kses($return).'</a>';
	}

	if( !empty($return) ){
		?>
		<div class="thsn-header-contactinfo"><?php echo thsn_esc_kses($return); ?></div>
		<?php
	}
}
}
...
function thsn_header_search(){
	...
	if( !empty($header_search) && $header_search=='1' ){
		?>
		<div class="thsn-header-search-btn"><a title="<?php echo esc_attr__( 'Search', 'itinc' )?>" href="#"><i class="thsn-base-icon-search-1"></i></a></div>
		<?php
	}
}
}
...
function thsn_pagination($wp_query_data=false){
	...
	$return  = '';
	$return .= thsn_esc_kses('<div class="clearfix"></div>');
	...
	if( $pagination!=NULL ){
		$big = 999999999; // need an unlikely integer
		$return .= '<div class="thsn-pagination"><div class="nav-links">';
		...
		$return .= '</div></div><!-- .thsn-pagination -->';
	}
	echo thsn_esc_kses($return);
}
}
...
if( !function_exists('thsn_preloader') ){
	function thsn_preloader(){
		$preloader = thsn_get_base_option('preloader');
		if( $preloader == true ){
			$preloader_img	= thsn_get_base_option('preloader-image');
			if( !empty($preloader_img) ){
				echo thsn_esc_kses('<div class="thsn-preloader" style="background-image:url('.esc_url( get_template_directory_uri() . '/images/loader'.esc_attr($preloader_img).'.svg'  ).')"></div>');
			}
		}
	}
}
...
function thsn_check_sidebar() {
	...
	if( is_page() ){
		...
		$page_meta = get_post_meta( $page_for_posts, 'thsn-sidebar', true );
		if( !empty($page_meta) && $page_meta!='global' ){
			$sidebar = $page_meta;
		}
	} else if( function_exists('is_woocommerce') && is_woocommerce() && !is_product() ){
	...
	if( !empty($atts['sortable']) && $atts['sortable']=='yes' ){
		...
		if( !empty($atts['from_category']) ){
			...
			foreach( $from_category as $catslug ){
				...
				$list .= '<li><a href="#" class="thsn-sortable-link" data-sortby="thsn-term-' . esc_attr($term->term_id) . '">' . esc_html($term->name) . '</a></li>';
			}
		} else {
			// all category
...
if( !function_exists('thsn_generateClassName') ){
function thsn_generateClassName($title) {
	// Transliterate Arabic to Latin (if Intl extension is available)
	if (class_exists('Transliterator')) {
		$transliterator = Transliterator::create('Any-Latin; Latin-ASCII');
		$title = $transliterator->transliterate($title);
	} else {
		// Fallback using iconv (might not work well for Arabic)
