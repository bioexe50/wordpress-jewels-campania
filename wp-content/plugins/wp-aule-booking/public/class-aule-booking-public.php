 ...
 * @subpackage WP_Aule_Booking/public
 */

class Aule_Booking_Public {
    ...
    public function enqueue_styles() {
        ...
        wp_enqueue_style(
            ...
            WP_AULE_BOOKING_PLUGIN_URL . 'public/css/aule-booking-public.css',
            array(),
            $this->version,
            'all'
        );
    ...
    public function enqueue_scripts() {
        ...
        wp_enqueue_script(
            $this->plugin_name . '-public',
            WP_AULE_BOOKING_PLUGIN_URL . 'public/js/aule-booking-public.js',
            array('jquery'),
            $this->version,
            true
        );
        ...
        wp_localize_script(
            ...
            array(
                ...
                'strings' => array(
                    ...
                    'confirm_booking' => __('Confermare la prenotazione?', 'aule-booking'),
                    'booking_sent' => __('Prenotazione inviata correttamente', 'aule-booking'),
                    'fill_required_fields' => __('Compila tutti i campi obbligatori', 'aule-booking'),
                    'invalid_email' => __('Indirizzo email non valido', 'aule-booking'),
                    'slot_unavailable' => __('Slot non più disponibile', 'aule-booking'),
                    'select_slot' => __('Seleziona uno slot', 'aule-booking'),
                    'privacy_required' => __('Devi accettare l\'informativa privacy', 'aule-booking'),
                    'recaptcha_required' => __('Completa la verifica reCAPTCHA', 'aule-booking')
                )
            )
        );
    ...
    public function shortcode_aule_list($atts) {
        ...
        ob_start();
        include WP_AULE_BOOKING_PLUGIN_DIR . 'public/partials/aule-booking-list.php';
        return ob_get_clean();
    }
    ...
    public function shortcode_search_form($atts) {
        $atts = shortcode_atts(array(
            'show_filters' => 'true',
            'redirect_to' => ''
        ), $atts, 'aule_booking_search');

        ob_start();
        include WP_AULE_BOOKING_PLUGIN_DIR . 'public/partials/aule-booking-search.php';
        return ob_get_clean();
    }
    ...
    public function ajax_submit_booking() {
        ...
        if (empty($booking_data['nome_richiedente']) || strlen($booking_data['nome_richiedente']) < 2) {
            $errors[] = __('Il nome deve contenere almeno 2 caratteri', 'aule-booking');
        }

        if (empty($booking_data['cognome_richiedente']) || strlen($booking_data['cognome_richiedente']) < 2) {
            $errors[] = __('Il cognome deve contenere almeno 2 caratteri', 'aule-booking');
        }

        if (empty($booking_data['email_richiedente']) || !is_email($booking_data['email_richiedente'])) {
            $errors[] = __('Indirizzo email non valido', 'aule-booking');
        }

        if (empty($booking_data['motivo_prenotazione']) || strlen($booking_data['motivo_prenotazione']) < 10) {
            $errors[] = __('Il motivo della prenotazione deve contenere almeno 10 caratteri', 'aule-booking');
        }

        if (!$booking_data['aula_id']) {
            $errors[] = __('Aula non valida', 'aule-booking');
        }

        if (empty($booking_data['data_prenotazione']) || empty($booking_data['ora_inizio']) || empty($booking_data['ora_fine'])) {
            $errors[] = __('Data e orari sono obbligatori', 'aule-booking');
        }

        // Verifica privacy
        if (empty($_POST['privacy_accepted'])) {
            $errors[] = __('Devi accettare l\'informativa privacy', 'aule-booking');
        }
        ...
        if (!empty($settings->abilita_recaptcha) && !empty($settings->recaptcha_secret_key)) {
            if (empty($_POST['recaptcha_token'])) {
                $errors[] = __('Verifica reCAPTCHA mancante', 'aule-booking');
            } else {
                $recaptcha_valid = $this->verify_recaptcha($_POST['recaptcha_token'], $settings->recaptcha_secret_key);
                if (!$recaptcha_valid) {
                    $errors[] = __('Verifica reCAPTCHA fallita', 'aule-booking');
                }
            }
        }
        ...
        if ($conflict) {
            wp_send_json_error(__('Slot già prenotato', 'aule-booking'));
        }

        // Controllo limiti temporali
        if (!$this->validate_booking_timing($booking_data, $settings)) {
            wp_send_json_error(__('Prenotazione fuori dai limiti consentiti', 'aule-booking'));
        }

        // Controllo limite prenotazioni per utente
        if (!$this->validate_user_booking_limit($booking_data, $settings)) {
            wp_send_json_error(__('Limite giornaliero di prenotazioni raggiunto', 'aule-booking'));
        }
        ...
        if ($booking_id) {
            ...
            wp_send_json_success(array(
                'message' => __('Prenotazione inviata correttamente', 'aule-booking'),
                'booking_id' => $booking_id,
                'stato' => $stato_iniziale
            ));
        } else {
            wp_send_json_error(__('Errore nell\'invio della prenotazione', 'aule-booking'));
        }
    }
    ...
    public function ajax_get_my_bookings() {
        ...
        if (empty($email)) {
            wp_send_json_error(__('Email richiesta', 'aule-booking'));
        }
                            ...
                            if (!$this->is_slot_booked($slot_start, $current_slot_end, $prenotazioni)) {
                                ...
                                if ($this->is_slot_bookable($slot_start, $settings)) {
                                    $events[] = array(
                                        ...
                                        'title' => __('Disponibile', 'aule-booking'),
