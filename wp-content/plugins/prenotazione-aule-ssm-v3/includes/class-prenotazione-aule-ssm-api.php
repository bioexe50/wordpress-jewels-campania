...
class Prenotazione_Aule_SSM_API {
    ...
    public function register_routes() {
        ...
        // GET /wp-json/prenotazione-aule-ssm/v1/aule - Lista di tutte le aule
        register_rest_route($this->namespace, '/aule', array(
            ...
            'args' => array(
                'stato' => array(
                    'description' => __('Filtra per stato aula', 'prenotazione-aule-ssm'),
                    'type' => 'string',
                    'enum' => array('attiva', 'non_disponibile', 'manutenzione'),
                    'sanitize_callback' => 'sanitize_text_field'
                ),
                'ubicazione' => array(
                    'description' => __('Filtra per ubicazione', 'prenotazione-aule-ssm'),
                    'type' => 'string',
                    'sanitize_callback' => 'sanitize_text_field'
                )
            )
        ...
        // GET /wp-json/prenotazione-aule-ssm/v1/aule/{id} - Singola aula
        register_rest_route($this->namespace, '/aule/(?P<id>\d+)', array(
            ...
            'args' => array(
                'id' => array(
                    'description' => __('ID dell\'aula', 'prenotazione-aule-ssm'),
                    'type' => 'integer',
        ...
        // GET /wp-json/prenotazione-aule-ssm/v1/availability/{aula_id} - Disponibilità aula
        register_rest_route($this->namespace, '/availability/(?P<aula_id>\d+)', array(
            ...
            'args' => array(
                ...
                'start' => array(
                    'description' => __('Data inizio (Y-m-d)', 'prenotazione-aule-ssm'),
                    'type' => 'string',
                    'format' => 'date',
                    'sanitize_callback' => 'sanitize_text_field',
                ...
                'end' => array(
                    'description' => __('Data fine (Y-m-d)', 'prenotazione-aule-ssm'),
                    'type' => 'string',
                    'format' => 'date',
                    'sanitize_callback' => 'sanitize_text_field',
                    'validate_callback' => function($param) {
        ...
        // POST /wp-json/prenotazione-aule-ssm/v1/booking - Crea nuova prenotazione
        register_rest_route($this->namespace, '/booking', array(
            ...
            'args' => array(
                ...
                'nome_richiedente' => array(
                    'description' => __('Nome del richiedente', 'prenotazione-aule-ssm'),
                    'type' => 'string',
                    'required' => true,
                    'minLength' => 2,
                    'sanitize_callback' => 'sanitize_text_field'
                ...
                'cognome_richiedente' => array(
                    'description' => __('Cognome del richiedente', 'prenotazione-aule-ssm'),
                    'type' => 'string',
                    'required' => true,
                    'minLength' => 2,
                    'sanitize_callback' => 'sanitize_text_field'
                ...
                'email_richiedente' => array(
                    'description' => __('Email del richiedente', 'prenotazione-aule-ssm'),
                    'type' => 'string',
                    'format' => 'email',
                    'required' => true,
                ...
                'motivo_prenotazione' => array(
                    'description' => __('Motivo della prenotazione', 'prenotazione-aule-ssm'),
                    'type' => 'string',
                    'required' => true,
                    'minLength' => 10,
                ...
                'data_prenotazione' => array(
                    'description' => __('Data prenotazione (Y-m-d)', 'prenotazione-aule-ssm'),
                    'type' => 'string',
                    'format' => 'date',
                    'required' => true,
                    'sanitize_callback' => 'sanitize_text_field'
                ...
                'ora_inizio' => array(
                    'description' => __('Ora inizio (H:i:s)', 'prenotazione-aule-ssm'),
                    'type' => 'string',
                    'required' => true,
                    'sanitize_callback' => 'sanitize_text_field'
                ),
                'ora_fine' => array(
                    'description' => __('Ora fine (H:i:s)', 'prenotazione-aule-ssm'),
                    'type' => 'string',
                    'required' => true,
                    'sanitize_callback' => 'sanitize_text_field'
                )
        ...
        // GET /wp-json/prenotazione-aule-ssm/v1/booking/{id} - Dettagli prenotazione
        register_rest_route($this->namespace, '/booking/(?P<id>\d+)', array(
            ...
            'args' => array(
                'id' => array(
                    'description' => __('ID della prenotazione', 'prenotazione-aule-ssm'),
                    'type' => 'integer',
                    'sanitize_callback' => 'absint'
                )
            )
        ...
        // PUT /wp-json/prenotazione-aule-ssm/v1/booking/{id}/status - Aggiorna stato prenotazione
        register_rest_route($this->namespace, '/booking/(?P<id>\d+)/status', array(
            ...
            'args' => array(
                ...
                'stato' => array(
                    'description' => __('Nuovo stato', 'prenotazione-aule-ssm'),
                    'type' => 'string',
                    'required' => true,
                ...
                'note_admin' => array(
                    'description' => __('Note admin', 'prenotazione-aule-ssm'),
                    'type' => 'string',
                    'sanitize_callback' => 'sanitize_textarea_field'
                )
            )
        ...
        // GET /wp-json/prenotazione-aule-ssm/v1/calendar/{aula_id} - Eventi calendario
        ...
        // GET /wp-json/prenotazione-aule-ssm/v1/prenotazioni - Lista prenotazioni (con filtri)
        register_rest_route($this->namespace, '/prenotazioni', array(
            ...
            'args' => array(
                'aula_id' => array(
                    'description' => __('Filtra per aula', 'prenotazione-aule-ssm'),
                    'type' => 'integer',
                    'sanitize_callback' => 'absint'
                ),
                'stato' => array(
                    'description' => __('Filtra per stato', 'prenotazione-aule-ssm'),
                    'type' => 'string',
                    'enum' => array('in_attesa', 'confermata', 'rifiutata', 'cancellata'),
                    'sanitize_callback' => 'sanitize_text_field'
                ),
                ...
                'per_page' => array(
                    'description' => __('Elementi per pagina', 'prenotazione-aule-ssm'),
                    'type' => 'integer',
                    'default' => 20,
                    'minimum' => 1,
                    'maximum' => 100,
                ...
                'page' => array(
                    'description' => __('Numero pagina', 'prenotazione-aule-ssm'),
                    'type' => 'integer',
                    'default' => 1,
                    'minimum' => 1,
                    'sanitize_callback' => 'absint'
    ...
    public function get_aula($request) {
        ...
        if (!$aula) {
            return new WP_Error('aula_not_found', __('Aula non trovata', 'prenotazione-aule-ssm'), array('status' => 404));
        }
    ...
    public function create_booking($request) {
        ...
        if (!$booking_id) {
            return new WP_Error('booking_creation_failed', __('Errore nella creazione della prenotazione', 'prenotazione-aule-ssm'), array('status' => 500));
        }
        ...
        return rest_ensure_response(array(
            'id' => $booking_id,
            'message' => __('Prenotazione creata correttamente', 'prenotazione-aule-ssm'),
    ...
    public function get_booking($request) {
        ...
        if (!$booking) {
            return new WP_Error('booking_not_found', __('Prenotazione non trovata', 'prenotazione-aule-ssm'), array('status' => 404));
        }
    ...
    public function update_booking_status($request) {
        ...
        if (!$result) {
            return new WP_Error('update_failed', __('Errore nell\'aggiornamento dello stato', 'prenotazione-aule-ssm'), array('status' => 500));
        }
        ...
        return rest_ensure_response(array(
            'message' => __('Stato aggiornato correttamente', 'prenotazione-aule-ssm'),
    ...
    public function get_calendar_events($request) {
        ...
        if (!$aula || $aula->stato !== 'attiva') {
            return new WP_Error('aula_not_available', __('Aula non disponibile', 'prenotazione-aule-ssm'), array('status' => 400));
        }
    ...
    private function validate_booking_data($booking_data) {
        ...
        if (!$aula || $aula->stato !== 'attiva') {
            return new WP_Error('invalid_aula', __('Aula non valida o non attiva', 'prenotazione-aule-ssm'), array('status' => 400));
        }
        ...
        if ($conflict) {
            return new WP_Error('booking_conflict', __('Slot già prenotato', 'prenotazione-aule-ssm'), array('status' => 409));
        }
        ...
        if ($booking_datetime <= $min_booking_time) {
            return new WP_Error('booking_too_early', __('Prenotazione troppo ravvicinata', 'prenotazione-aule-ssm'), array('status' => 400));
        }
        ...
        if ($booking_datetime > $max_booking_time) {
            return new WP_Error('booking_too_far', __('Prenotazione troppo lontana nel futuro', 'prenotazione-aule-ssm'), array('status' => 400));
        }
    ...
    public function check_manage_permission($request) {
        return current_user_can('manage_prenotazione_aule_ssm');
    }
}