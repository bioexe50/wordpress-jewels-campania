...
foreach ( $menus as $menu ) {
	if ( $menu->name == 'Main Menu' ) {
		$menu_ids['themesion-top'] = $menu->term_id;

	} else if ( $menu->name == 'Footer Menu' ) {
		$menu_ids['themesion-footer'] = $menu->term_id;
...
if ( is_dir( $export_content_path ) ) {
	...
	$stylefolder = basename(get_theme_mod('thsn_itinc_site_style',$this->get_default_theme_style()));
	if($stylefolder){
...
?>
	<h1><?php echo esc_html__('Export Done', 'itinc');?>:</h1>
	<p>
		<?php echo esc_html__('Export content has been placed into below folder.', 'itinc');?><br>
		<em><?php echo esc_html($export_content_path);?></em>
	</p>
	<p>
		<?php echo esc_html__('Stock images have been copied into below folder for faster theme install.', 'itinc');?><br>
