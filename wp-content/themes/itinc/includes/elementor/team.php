 ...
 */
class THSN_TeamElement extends Widget_Base{

 	// The get_name() method is a simple one, you just need to return a widget name that will be used in the code.
	public function get_name() {
		return 'thsn_team_element';
	}

	// The get_title() method, which again, is a very simple one, you need to return the widget title that will be displayed as the widget label.
	public function get_title() {
		return esc_attr__( 'ITinc Team Member Element', 'itinc' );
	}
	...
	protected function register_controls() {
		...
		$this->add_control(
			'from_category',
			[
				'label' => esc_attr__( 'Show Post from selected Team Category?', 'itinc' ),
		...
		$this->add_control(
			'sortable',
			[
				'label' => esc_attr__( 'Show Sortable Team Category ?', 'itinc' ),
				'description' => esc_attr__( 'Select YES to show sortable Team Category.', 'itinc' ),
				'type' => Controls_Manager::SWITCHER,
				'label_on' => esc_attr__( 'Yes', 'itinc' ),
				'label_off' => esc_attr__( 'No', 'itinc' ),
				'return_value' => 'yes',
		...
		$this->add_control(
			...
			[
				...
				'description' => esc_attr__( 'Select YES to Show pagination links below Team boxes.', 'itinc' ),
				'type' => Controls_Manager::SWITCHER,
				'label_on' => esc_attr__( 'Yes', 'itinc' ),
				'label_off' => esc_attr__( 'No', 'itinc' ),
				'return_value' => 'yes',
		...
		$this->add_control(
			'style',
			[
				'label'			=> esc_attr__( 'Select Team Member View Style', 'itinc' ),
				'description'	=> esc_attr__( 'Slect Team Member View style.', 'itinc' ),
				'type'			=> 'thsn_imgselect',
				'label_block'	=> true,
				'thumb_width'	=> '110px',
				'default'		=> '1',
				'options'		=> thsn_element_template_list( 'team', true ),
			]
		);
		$this->end_controls_section();
	...
	protected function render() {
		...
		if ( $posts->have_posts() ) {
			...
			if( function_exists('thsn_sortable_category') ){
				$sortable_html = thsn_sortable_category( $settings, 'thsn-team-group' );
			}
			?>
			...
			<div class="thsn-ele-header-area">
				...
				<?php echo thsn_esc_kses($sortable_html); ?>
			</div>
			<?php } ?>
			...
			if( $style=='1' ){
				$return .= '<div class="container thsn-container"></div>';
			}
			...
			while ( $posts->have_posts() ) {
				...
				if( file_exists( locate_template( '/theme-parts/team/team-style-'.esc_attr($style).'.php', false, false ) ) ){
					...
					$boxes .= thsn_element_block_container( array(
						'position'	=> 'start',
						'column'	=> $columns,
						'cpt'		=> 'team',
						'taxonomy'	=> 'thsn-team-group',
		...
		// Special design for Style-1
		if( $style=='1' ){

			$car_nav = '';
			if( !empty($settings['view-type']) && $settings['view-type']=='carousel' ){
				$car_nav = '<div class="thsn-carousel-navs">
			...
			$carousel_area_class = '';
			if( !empty($settings['view-type']) && $settings['view-type']=='carousel' ){
				$carousel_area_class = 'thsn-team-1-carousel-area';
			}

			$return .= '
			<div class="thsn-team-1-container-area ' . esc_attr($carousel_area_class) . '">
				<div class="container">
					<div class="thsn-ele-header-area">
						' . thsn_heading_subheading($settings, false) . '
						' . thsn_esc_kses($sortable_html) . '
					</div>
					' . thsn_esc_kses($car_nav) . '
				</div>
				<div class="thsn-team-1-inner">
			...
			$return .= thsn_esc_kses('<div class="thsn-element-posts-wrapper row multi-columns-row">'.$boxes.'</div>');
		}
	...
	protected function select_category() {
		$category = get_terms( array( 'taxonomy' => 'thsn-team-group', 'hide_empty' => false ) );
	  	$cat = array();
...
Plugin::instance()->widgets_manager->register( new THSN_TeamElement() );