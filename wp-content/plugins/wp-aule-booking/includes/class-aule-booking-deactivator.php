...
class Aule_Booking_Deactivator {
    ...
    private static function unschedule_cron_events() {
        wp_clear_scheduled_hook('aule_booking_cleanup_expired');
        wp_clear_scheduled_hook('aule_booking_send_reminders');
        wp_clear_scheduled_hook('aule_booking_weekly_report');
    }
    ...
    private static function clear_transients() {
        delete_transient('aule_booking_stats');
        delete_transient('aule_booking_availability_cache');
    }
}
 ...
 * @since 1.0.0
 */
class Aule_Booking_Uninstaller {
    ...
    public static function uninstall() {
        ...
        if (get_option('aule_booking_delete_data_on_uninstall', false)) {
            self::drop_tables();
        }
    ...
    private static function drop_tables() {
        ...
        $tables = array(
            $wpdb->prefix . 'aule_booking_prenotazioni',
            $wpdb->prefix . 'aule_booking_slot_disponibilita',
            $wpdb->prefix . 'aule_booking_aule',
            $wpdb->prefix . 'aule_booking_impostazioni'
        );
    ...
    private static function remove_options() {
        delete_option('aule_booking_version');
        delete_option('aule_booking_installed_date');
        delete_option('aule_booking_delete_data_on_uninstall');
    ...
    private static function remove_capabilities() {
        ...
        if ($role) {
            $role->remove_cap('manage_aule_booking');
            $role->remove_cap('edit_aule_booking');
            $role->remove_cap('delete_aule_booking');
            $role->remove_cap('view_aule_booking_reports');
        }
    }
}