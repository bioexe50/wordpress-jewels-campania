...
class Prenotazione_Aule_SSM_Multi_Slot {
    ...
    public function ajax_multi_booking() {
        ...
        try {
            ...
            // Carica classe Email ufficiale per notifiche
            ...
            // Ottieni ID ultima prenotazione inserita per inviare email template
            $first_booking_id = $wpdb->get_var($wpdb->prepare(
                "SELECT id FROM $table_prenotazioni
                 WHERE gruppo_prenotazione = %s
                 ORDER BY id ASC LIMIT 1",
                $gruppo_prenotazione
            ));

            if ($first_booking_id) {
                // Invia email utente (conferma prenotazione ricevuta)
                $this->send_multi_booking_confirmation_email($email, $nome, $cognome, $selected_slots, $motivo);

                // Invia notifica admin per approvazione
    ...
    /**
     * Invia email di conferma all'utente per prenotazione multi-slot
     * Questa è un'email semplificata che conferma la ricezione, non l'approvazione
     */
    private function send_multi_booking_confirmation_email($email, $nome, $cognome, $slots, $motivo) {
        $to = $email;
        $subject = sprintf('[%s] Prenotazione Ricevuta', get_bloginfo('name'));
