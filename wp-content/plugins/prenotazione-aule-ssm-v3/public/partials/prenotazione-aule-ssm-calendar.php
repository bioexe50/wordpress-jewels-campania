...
$calendar_id = 'prenotazione-aule-ssm-calendar-' . $aula_id;
$allow_booking = ($atts['allow_booking'] === 'true');
$show_legend = ($atts['show_legend'] === 'true');
?>

<div class="prenotazione-aule-ssm-wrapper" data-aula-id="<?php echo esc_attr($aula_id); ?>">

    <!-- Intestazione -->
    <div class="prenotazione-aule-ssm-header">
        <h3 class="aula-title">
        ...
        <div class="aula-info">
            ...
            <?php if ($aula->capienza > 0): ?>
                <span class="aula-capacity">
                    ...
                    <?php printf(__('Capienza: %d persone', 'prenotazione-aule-ssm'), $aula->capienza); ?>
                </span>
            <?php endif; ?>
        </div>
        ...
        <?php if (!empty($aula->attrezzature)): ?>
            ...
            <?php if (is_array($attrezzature) && !empty($attrezzature)): ?>
                <div class="aula-facilities">
                    <h4><?php _e('Attrezzature disponibili:', 'prenotazione-aule-ssm'); ?></h4>
    ...
    <!-- Legenda -->
    <?php if ($show_legend): ?>
        <div class="prenotazione-aule-ssm-legend">
            ...
            <div class="legend-item">
                ...
                <span class="legend-label"><?php _e('Occupato', 'prenotazione-aule-ssm'); ?></span>
            </div>
            <div class="legend-item">
                <span class="legend-color legend-pending"></span>
                <span class="legend-label"><?php _e('In Attesa', 'prenotazione-aule-ssm'); ?></span>
            </div>
        </div>
    <?php endif; ?>

    <!-- Calendario -->
    <div id="<?php echo esc_attr($calendar_id); ?>" class="prenotazione-aule-ssm-calendar" style="height: <?php echo esc_attr($atts['height']); ?>px;"></div>

    <!-- Loading overlay -->
    <div class="prenotazione-aule-ssm-loading" id="loading-<?php echo esc_attr($aula_id); ?>">
        <div class="loading-spinner">
            <i class="fas fa-spinner fa-spin"></i>
            <p><?php _e('Caricamento calendario...', 'prenotazione-aule-ssm'); ?></p>
        </div>
    </div>
</div>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="booking-slot-summary">
                    <h6><?php _e('Slot selezionato:', 'prenotazione-aule-ssm'); ?></h6>
                ...
                <form id="bookingForm-<?php echo esc_attr($aula_id); ?>" class="booking-form">
                    ...
                    <input type="hidden" name="nonce" value="<?php echo wp_create_nonce('prenotazione_aule_ssm_public_nonce'); ?>">
                    ...
                    <div class="mb-3 form-check">
                        ...
                        <label class="form-check-label" for="privacy_accepted">
                            <?php _e('Accetto l\'informativa privacy e il trattamento dei dati personali *', 'prenotazione-aule-ssm'); ?>
                        </label>
                        <div class="invalid-feedback">
...
.prenotazione-aule-ssm-calendar {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    var calendar = new FullCalendar.Calendar(calendarEl, {
        ...
        eventSources: [{
            url: prenotazione_aule_ssm_ajax.ajax_url,
            method: 'POST',
            extraParams: {
                action: 'aule_check_availability',
                aula_id: auleId,
                nonce: prenotazione_aule_ssm_ajax.nonce
            },
            failure: function() {
    ...
    function openBookingModal(event) {
        ...
        if (prenotazione_aule_ssm_ajax.settings.recaptcha_enabled && prenotazione_aule_ssm_ajax.settings.recaptcha_site_key) {
            var recaptchaContainer = document.getElementById('recaptcha-container');
            recaptchaContainer.style.display = 'block';
            recaptchaContainer.querySelector('.g-recaptcha').setAttribute('data-sitekey', prenotazione_aule_ssm_ajax.settings.recaptcha_site_key);
        }
    ...
    document.getElementById('submitBooking').addEventListener('click', function() {
        ...
        if (!document.getElementById('privacy_accepted').checked) {
            showError(prenotazione_aule_ssm_ajax.strings.privacy_required);
            return;
        }
        ...
        // Gestione reCAPTCHA
        if (prenotazione_aule_ssm_ajax.settings.recaptcha_enabled) {
            if (typeof grecaptcha !== 'undefined') {
                grecaptcha.execute(prenotazione_aule_ssm_ajax.settings.recaptcha_site_key, {action: 'booking'})
                .then(function(token) {
    ...
    function submitBookingRequest(formData, submitBtn, btnText, btnSpinner) {
        fetch(prenotazione_aule_ssm_ajax.ajax_url, {
            method: 'POST',
            body: formData
        })
        ...
        .catch(error => {
            resetSubmitButton(submitBtn, btnText, btnSpinner);
            showError(prenotazione_aule_ssm_ajax.strings.error);
...
<?php include PRENOTAZIONE_AULE_SSM_PLUGIN_DIR . 'public/partials/prenotazione-aule-ssm-multi-slot-modal.php'; ?>
