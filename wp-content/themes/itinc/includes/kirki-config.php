<?php
Kirki::add_config( 'themesion_itinc_base_options', array(
	'capability'  => 'edit_theme_options',
	'option_type' => 'theme_mod',
) );
// Panel
Kirki::add_panel( 'itinc_base_options', array(
    'priority'    => 10,
    'title'       => esc_attr__( 'ITinc Options', 'itinc' ),
    'description' => esc_attr__( 'ITinc theme options', 'itinc' ),
) );
...
foreach( $kirki_options_array as $section_id=>$data ){
	...
	if( !empty($data['section_fields']) ){
		foreach( $data['section_fields'] as $field ){
			...
			Kirki::add_field( 'themesion_itinc_base_options', $field );
		}
	}
}
}
 ...
 * @return array
 */
if( !function_exists('thsn_kirki_demo_configuration') ){
function thsn_kirki_demo_configuration( $config ) {
	...
	return wp_parse_args( array(
		...
		'description'	=> esc_attr__( 'By Themesion Team.', 'itinc' ),
	), $config );
}
}
add_filter( 'kirki_config', 'thsn_kirki_demo_configuration' );
